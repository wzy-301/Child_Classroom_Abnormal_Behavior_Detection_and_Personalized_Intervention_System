...
- **视频进度条与跳转**：支持拖动进度条跳转到任意帧，跳转时自动暂停/恢复播放
- **暂停/继续按钮**：检测过程中可随时暂停分析，方便查看某一帧结果
![alt text](image.png)
...
- **主模型**：Prototypical Networks（少样本学习）
  ...
  - 特征空间 **L2 归一化 + 余弦相似度** 分类
  - **验证集早停**：每类固定抽取验证样本，自动保存最佳模型
...
- **4 阶段智能绘制流程**：
  1. **检测阶段**：收集所有目标框和标签尺寸
  2. **计算 Padding**：若标签会超出画面上边缘或右边缘，计算所需扩展量
  3. **扩大画布**：通过 `cv2.copyMakeBorder` 扩展画布，填充浅灰色背景
  4. **坐标调整与绘制**：调整所有目标框坐标，绘制框和标签
...
- **图片检测**：单张图片上传分析，即时确认异常（无 3 秒延迟）
- **视频处理**：视频文件检测并保存处理结果，支持进度条跳转
- **实时性能监控**：FPS、检测耗时、目标数量实时显示
![alt text](image-4.png)

### 8. 智能干预系统
- **3 秒行为确认机制**（视频/摄像头模式）：异常行为需持续出现 3 秒才确认并弹窗，避免瞬时抖动误报
- **图片即时确认**（图片模式）：单张图片检测即时确认异常
...
| patience | 300 | 早停耐心值（连续 300 轮验证不提升则停） |
...
**训练流程**：
1. 每类固定抽取若干张作为验证集，其余训练
2. Backbone 冻结，只优化 projector
3. 定期验证，保存最佳模型到 `protonet_best.pth`
4. 验证性能连续不提升 → 自动早停
...
|------|------|
| 📷 摄像头 | 开启实时摄像头检测 |
| 🖼️ 图片检测 | 选择单张图片进行检测（即时确认异常） |
| 🎬 视频检测 | 选择视频文件仅实时播放检测（不保存） |
| 💾 视频保存 | 选择视频并保存处理结果 |
| ⏹️ 停止检测 | 停止当前检测任务 |
...
系统采用 **7 层保守策略**，核心目标：**宁可漏报，不可误报**，优先保护正常行为不被误判：

1. **手机检测优先**：YOLO 检测到手机物体（class 67）且满足空间约束 → 直接判定 `play_phone`
2. **play_phone 打折**：未检测到手机 → `play_phone` 相似度 × 0.5，降低无手机时的虚高
3. **强 normal 绝对值保护（核心）**：当 `normal ≥ 0.28`，除非最高异常类显著领先（差距 ≥ 0.12），否则强制判 `normal`。这是防止 `looking_around`/`whispering` 擦线误判的最强屏障
4. **通用模糊保护**：若最高异常类只比 `normal` 高 < 0.10，且 `normal ≥ 0.18` → 返回 `normal`
5. **looking_around & whispering 专属降级**：
   - 若第二名是 `normal` 且差距 < 0.06 → 返回 `normal`
   - 若第二名也是异常但差距 < 0.04 → 模型整体混乱，返回 `normal`
6. **play_phone 专属降级**：打折后仍最高，检查第二名；若第二名与 `normal` 接近 → 降级为 `normal` 或第二名
7. **全局低置信度兜底**：如果所有类别中的最高分都低于 0.30，说明模型整体不确定，强制判 `normal`

**阈值过滤（贯穿各层）**：
- `play_phone` 必须超过 0.50（阈值 0.45 + 0.05），否则退回 `normal`
- `whispering`/`looking_around` 必须超过 0.38，否则退回 `normal`（代码内部 fallback 为 0.42，更严格）
- `stand`/`lie`/`fight` 必须超过 0.30，否则退回 `normal`

**`detect_and_draw` 额外保护**：
- 若 `classify_crop_protonet` 输出 `play_phone`，但 `play_phone` 与第二名的差距 < `fallback_gap`（默认 0.15），则降级为第二名
...
- **训练优化**：
  ...
  - 训练/评估统一使用**余弦相似度**，度量更稳定
  - 验证集早停：每类固定抽取验证样本，自动保存最佳模型
  - 原型归一化：生成 GUI 原型时均值后重新 L2 归一化
  - CosineAnnealingLR 替代 StepLR，学习率衰减更平滑
- **分类逻辑重构**：
  - **7 层保守保护策略**（新增全局低置信度兜底）
  - 强化 normal 行为保护：`normal ≥ 0.28` 时异常类必须领先 ≥ 0.12
  - 大幅提高 `whispering`/`looking_around` 判定门槛
  - `play_phone` 专属二次降级移至 `detect_and_draw`，避免重复打压
- **UI 增强**：
  - 新增暂停/继续按钮，支持视频播放中随时暂停分析
  - 新增视频进度条，支持拖动跳转到任意帧
  - 新增"视频检测"模式（仅实时播放，不保存文件）
  - 3 秒行为确认机制：视频/摄像头模式下异常需持续 3 秒才弹窗
  - 图片检测模式即时确认异常，无需等待
- **阈值更新**：同步余弦相似度范围，重新校准所有类别阈值

### v1.4.0 
- **标签防截断**：4 阶段智能绘制流程（检测→计算→扩展→绘制）
...
- **分类方式**：**余弦相似度**（L2 归一化后点积，数值范围 0~1，更稳定）
- **原型生成**：每类样本特征平均 + **重新 L2 归一化**，确保与训练度量一致
- **验证策略**：固定验证集，早停 patience=300
...
- **标签防截断**：4 阶段自动扩展画布，确保标签完整显示
- **人次统计**：同一帧多学生同类异常按人数累计
- **视频跳转**：支持进度条拖动跳转到任意帧
- **暂停恢复**：检测过程中可随时暂停/继续
- **行为确认**：视频/摄像头 3 秒确认，图片即时确认
...
14. 视频模式下支持拖动进度条跳转，跳转时自动暂停分析，跳转完成后自动恢复
15. 空格键为暂停/继续快捷键，仅在检测运行中有效
