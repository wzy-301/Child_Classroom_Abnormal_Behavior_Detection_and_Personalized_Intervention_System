...
- **双重验证机制**：
  - **重叠判断**：手机框与人框的覆盖比例（`cover_ratio = inter_area / phone_area`）> 0.3，适应手持场景（手机完全在人框内部时标准 IoU 极低）
  - **距离判断**：手机中心与人中心距离 < 人体高度 × `phone_dist_multiplier`（默认 1.2），且手机中心位于人框水平范围 ±0.5 倍宽度内，防止邻座误伤
  - **垂直范围**：手机位于人体 20%~90% 高度区域（适应坐姿手持），且手机中心不超过人体高度 85%（排除桌上手机）
...
系统采用 **5 层保守策略**，优先判为 normal，防止课堂场景下的过度告警：

1. **手机检测优先**：YOLO 检测到手机物体（class 67）且满足空间约束 → 直接判定 play_phone
   - 覆盖比例 > 0.3（手机被人框覆盖的程度）
   - 或距离 < 人体高度 × 1.2，且位于水平 ±0.5 倍人宽、垂直 20%~90% 范围内
2. **play_phone 打折**：未检测到手机 → play_phone 相似度 × 0.5
3. **通用模糊保护（第2层）**：若最高异常类只比 normal 高 < 0.08，且 normal ≥ 0.20 → 返回 normal
4. **normal 优先策略（第3层）**：若 normal ≥ 0.35 且最高异常类与 normal 差距 < 0.1 → 返回 normal
5. **play_phone 专属降级（第4层）**：play_phone 打折后仍最高，但第二名与 normal 差距 < 0.1 → 降级为第二名或 normal
6. **阈值过滤（第5层）**：所有异常类必须超过各自阈值，play_phone 额外要求超过阈值 + 0.05，否则退回 normal
...
| **有手机但判为 normal** | 1. 检查手机检测日志（`📱 本帧检测到手机数量`）<br>2. 降低手机检测置信度（代码中 `conf=0.30` 可改为 `0.25`）<br>3. 放宽 `phone_dist_multiplier` 或 `phone_iou_threshold` |
...
6. **调整保守策略**：若某场景 normal 误判率仍高，可在 `config.json` 中：
   - 降低 `normal_min_for_fix2`（如从 0.35 改为 0.30）
   - 增大 `fix2_gap_threshold`（如从 0.1 改为 0.12）
   - 增大通用 gap 阈值（修改代码中 `0.08` 为 `0.10`）

## 更新日志

### v1.4.0 (2026-04-23)
- **标签防截断**：检测阶段自动计算标签所需空间，通过 `cv2.copyMakeBorder` 扩展画布，确保标签完整显示
- **异常按人数统计**：同一帧多个学生出现同类异常时，统计面板按人次计数（如 `lie: 3次`），干预面板显示 `×N人`
- **视频保存兼容**：视频写入时自动裁切 padding，避免编码错误
- **手机检测优化**：
  - 改用 `cover_ratio`（手机被人体覆盖比例）替代标准 IoU，适应手持场景
  - 增加水平范围约束（±0.5 倍人宽），防止邻座误伤
  - 垂直范围放宽至 20%~90%，适应坐姿手持
  - 手机中心不超过人体 85% 高度，排除桌上手机
- **5 层保守分类策略**：新增通用模糊保护（gap < 0.08），play_phone 阈值加 0.05 缓冲
...
- **重叠判断**：`cover_ratio = inter_area / phone_area > 0.3`（手机被人体覆盖的比例）
- **距离判断**：手机中心与人中心距离 < 人体高度 × `phone_dist_multiplier`（默认 1.2）
- **水平约束**：手机中心位于人框水平范围 ±0.5 倍宽度内
- **垂直约束**：手机位于人体 20%~90% 高度，且中心不超过 85%
